...
def show_post_issue():
    ...
    with ui.element().style(
        ...
        "w-full min-h-screen flex flex-row justify-between items-center py-20 mt-10"
    ):
        with ui.column().classes("w-1/2 flex flex-col justify-center items-center"):
            ui.label("Report a New Issue").classes("text-3xl font-semibold mb-2")

            with ui.card().classes(
                "w-[80%] flex flex-col justify-center items-center mt-8 mb-8"
            ):
                title = (
        ...
        with ui.column().classes('w-1/2 flex justify-center items-center fixed bottom-0 right-0'):
            ui.image("/assets/post_issue.png").classes('w-3/4 h-auto')
