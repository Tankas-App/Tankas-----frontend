    ...
    with ui.element("main").classes("w-full h-screen flex justify-center items-center").style('font-family: "Raleway", serif;'):
        with ui.card().classes('w-[40%] h-[80%] flex items-center'):
            ...
            with ui.element('div').classes('flex flex-col justify-center items-center'):
                ui.label("Volunteer & Earn Points").classes('text-3xl p-4 font-semibold')
                ui.label("Help your community by volunteering to fix reported issues. Earn points for your contributions and climb the leaderboard!").classes('text-gray-500 w-[80%]')
            with ui.element('div').classes('w-full flex flex-row justify-around items-center'):
                ui.button('Previous', on_click=lambda: ui.navigate.back()).props('flat dense no-caps').classes('bg-green-200 px-5 font-semibold text-black rounded-2xl')
                ui.button('Next', on_click=lambda: ui.navigate.to('/tutorial3')).props('flat dense no-caps').classes('bg-green-500 px-5 font-semibold text-black rounded-2xl')