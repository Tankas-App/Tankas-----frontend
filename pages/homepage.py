...
def homepage():
    ...
    with ui.element('main').classes('w-full h-screen flex flex-col justify-center items-center bg-[url("/assets/wom.jpg")] bg-cover bg-center').classes('').style('font-family: "Raleway", serif; color: #2E86AB'):
        with ui.element('div').classes('text-white bg-black/50 w-full h-full flex flex-col justify-center items-center'):
            ui.label("Clean Together, Thrive Together").style('font-family: "Archivo Black", sans-serif;').classes('text-8xl mb-8 px-10 text-center w-[80%]')
            ui.label("Join Tankas, the gamified sanitation app that turns community improvement into a fun, rewarding adventure. Earn points, badges, and recognition for making your neighborhood cleaner and greener.").classes('w-[40%] text-lg text-white mb-8')
