...
def _run_signup(data):
    return requests.post(f"{base_url}/api/auth/signup", data=data)
...
def show_signup_page():
    ...
    with ui.element('div').classes('w-full h-full flex flex-col justify-center items-center py-20'):
        ...
        with ui.card().classes('w-[30%] flex flex-col items-center shadow-lg'):
            ...
            display_name = ui.input(placeholder='Enter your display name').classes('w-full').props('outlined')
            ...
            _signup_btn = ui.button("Sign Up", on_click=lambda: _signup(data={"username": username.value,
                                                                "email": email.value,
                                                                "password": password.value,
                                                                "display_name": display_name.value})).props('flat dense no-caps').classes('w-full')
        with ui.row():
